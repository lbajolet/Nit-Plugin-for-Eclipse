...
public class NitCompilerMessage {
	...
	private int[] getInfos(String midStringInCompilerMessage){
		...
		while(midStringInCompilerMessage.charAt(currIndexInString) != ',' && currIndexInString < midStringInCompilerMessage.length()){
		...
		while(midStringInCompilerMessage.charAt(currIndexInString) != '-' && currIndexInString < midStringInCompilerMessage.length()){
			startIndex.append(midStringInCompilerMessage.charAt(currIndexInString));
			currIndexInString ++;
		}
		...
		while(currIndexInString < midStringInCompilerMessage.length()){
			if(midStringInCompilerMessage.charAt(currIndexInString) != '-'){
				endIndex.append(midStringInCompilerMessage.charAt(currIndexInString));
			}
			currIndexInString ++;
		}

...
public class NitPreferencePage extends FieldEditorPreferencePage implements
		...
		protected String changePressed() {
			...
			if(dirName != null){
				...
				if(nitComp.exists() && nitComp.isFile()){
					...
					NitActivator.getDefault().getPreferenceStore().setValue("compilerLocation", dirName + fileSeparator + "bin" + fileSeparator + "nitc");
				}
				
				if(dirLib.exists() && dirLib.isDirectory()){
					libFieldEditor.setStringValue(dirName + fileSeparator + "lib");
					NitActivator.getDefault().getPreferenceStore().setValue("NitLibEditor", dirName + fileSeparator + "lib");
				}
			}
	...
	protected void createFieldEditors() {
		...
		libFieldEditor = new DirectoryFieldEditor("NitLibEditor",
				"Nit StdLib Folder Location", getFieldEditorParent());
		addField(libFieldEditor);

		fileFieldEditor = new FileFieldEditor("compilerLocation",

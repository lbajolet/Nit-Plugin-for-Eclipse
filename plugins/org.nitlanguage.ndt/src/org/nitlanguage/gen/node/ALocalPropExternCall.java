/* This file was generated by SableCC (http://www.sablecc.org/). */

package org.nitlanguage.gen.node;

import org.nitlanguage.gen.analysis.Analysis;

@SuppressWarnings("nls")
public final class ALocalPropExternCall extends PExternCall
{
    private PMethid _methid_;

    public ALocalPropExternCall()
    {
        // Constructor
    }

    public ALocalPropExternCall(
        @SuppressWarnings("hiding") PMethid _methid_)
    {
        // Constructor
        setMethid(_methid_);

    }

    @Override
    public Object clone()
    {
        return new ALocalPropExternCall(
            cloneNode(this._methid_));
    }

    @Override
    public void apply(Switch sw)
    {
        ((Analysis) sw).caseALocalPropExternCall(this);
    }

    public PMethid getMethid()
    {
        return this._methid_;
    }

    public void setMethid(PMethid node)
    {
        if(this._methid_ != null)
        {
            this._methid_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._methid_ = node;
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._methid_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._methid_ == child)
        {
            this._methid_ = null;
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        if(this._methid_ == oldChild)
        {
            setMethid((PMethid) newChild);
            return;
        }

        throw new RuntimeException("Not a child.");
    }
}

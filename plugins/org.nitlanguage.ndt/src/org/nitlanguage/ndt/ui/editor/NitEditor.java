...
/**
 * The editor for nit, bound to the completion methods and the highlighting
 * functionalities
 * 
...
public class NitEditor extends TextEditor {
	...
	public static final String NIT_EDITOR_ID = "org.nitlanguage.ndt.ui.editor.NitEditor";
	...
	/**
	 * Returns a map containing the text selected in the editor line by line
	 * with the associated line number as key
	 * 
	 * @return
	 */
		...
		if (startl != endl) {
			...
			for (int i = startl; i <= endl; i++) {
				try {
					...
					lines.put(
							i,
							doc.get(line_infos.getOffset(),
									line_infos.getLength()));
	...
	public String lineAt(int index) {
		try {
			...
			return getDocument().get(lineRegion.getOffset(),
	...
	public boolean comment(Set<Integer> lines) {
		...
		for (int line : lines) {
			try {
				...
				String value = doc.get(line_infos.getOffset(),
						line_infos.getLength());
				doc.replace(line_infos.getOffset(), line_infos.getLength(),
						Character.toString(comment) + value);
			} catch (BadLocationException e) {
				return false;
			}
		}
	...
	/**
	 * Remove the comment clause at the beginning of each line specified by the
	 * set
	 * 
	 * @param lines
	 * @return
			...
			try {
				...
				for (int i = 0; i < value.length(); i++) {
					if (Character.isWhitespace(value.charAt(i)))
						...
						String new_value = value.substring(0, i)
								+ value.substring(i + 1, value.length());
						doc.replace(line_infos.getOffset(),
								line_infos.getLength(), new_value);
					}
				}
		...
		try {
			...
			for (IEditorReference ed : editors) {
				if (ed.getEditorInput() instanceof FileEditorInput) {
					...
					if (fileBoundToIDocument.getFileExtension().contains(
							NIT_FILE_EXTENSION)) {
	...
	public void clean() {
		...
		for (int i = 0; i < nbSrcLines; i++) {
			try {
				...
				DeclarationType type_decl = Declaration.evaluateDeclaration(
						rawLine, lineAt(i + 1));
				...
				if (!isModuleSet) {
					...
					if (type_decl == DeclarationType.MULTILINE_FUN) {
						...
						String splitedLine = rawLine.trim().substring(0,
								rawLine.length() - 3);
	...
	public boolean correct_indent() {
		...
		for (int i = 0; i < nbLines; i++) {
			...
			try {
				...
				type_decl = Declaration.evaluateDeclaration(rawLine,
						lineAt(i + 1));
			} catch (BadLocationException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			...
			if (type_decl == null)
				return false;
			/*
			 * if(type_decl == DeclarationType.MODULE || type_decl ==
			 * DeclarationType.OTHER || type_decl == DeclarationType.COMMENT ||
			 * type_decl == DeclarationType.INLINE_FUN || type_decl ==
			 * DeclarationType.INLINE_IF) { continue; }
			 */
			if (type_decl == DeclarationType.MULTILINE_FUN_CLEAN
				...
				try {
					...
					// *clean means that do keyword is on a new line according
					// to good practice
					// then next line (do) must have the same offset that
					// multiline fun declaration
					if (dec.getType() == DeclarationType.MULTILINE_FUN_CLEAN)
						lineBegin++;
					lineEnd = doc.getLineOfOffset(dec.getBeginOffset()
							+ realOffset);
				} catch (BadLocationException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				...
				/*
				 * Block begin : dec.getBeginOffset(), line = lineBegin Block
				 * end : dec.getBeginOffset() + realOffset, line = lineEnd Line
				 * to shift : (lineBegin+1) to lineEnd-1
				 */

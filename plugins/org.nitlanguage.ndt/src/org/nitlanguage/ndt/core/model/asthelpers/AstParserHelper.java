...
/**
 * Class used to simplify the treatments on the AST generated by the Nit Parser
 * 
...
public class AstParserHelper {
	...
	public Start getAstForDocument(AStdImport imp, IFile fileToSeekFrom) {
		...
		while (separatedPath.length > 1) {
			...
			File toCheck = new File(sah.join(separatedPath, "/")
	...
	private void saveStartNodeInAST(Start node, IFile fileBound) {
		try {
			((NitNature) fileBound.getProject().getNature(
					PluginParams.NATURE_ID)).getAstReposit().addOrReplaceAST(
					fileBound.getName(), node);
		} catch (Exception e) {
			if (NitActivator.DEBUG_MODE)
				e.printStackTrace();
		}
	...
	private void saveStartNodeInAST(Start node, IDocument fileBound) {
		...
		try {
			docName = (AModuledecl) this.getModuleOfAST(node).getModuledecl();
			...
			for (IEditorReference editor : PlatformUI.getWorkbench()
				...
				if (editor.getEditorInput() instanceof FileEditorInput) {
					...
					if (fileName.equals(modName.trim()
							+ PluginParams.NIT_EXTENSION)) {
	...
	private Lexer getLexForSource(IFile file) {
		...
		try {
			Path test_path = new Path(NitActivator.getWorkspacePath()
					.toOSString() + file.getFullPath().toString());
	 ...
	 * @throws Exception 
	 */
	public boolean checkIfExecutable(Start node) throws Exception {
